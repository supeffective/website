...
// const _nodeModulesDir = path.join(__dirname, '..', '..', 'node_modules')
// const _defaultPrismaClientDir = path.resolve(path.join(_nodeModulesDir, '@prisma', 'client'))
const _defaultPrismaClientDir = path.resolve(path.join(__dirname, 'generated', 'client'))
const _defaultPrismaSchemaDir = path.resolve(path.join(__dirname))
...
async function getPrismaDir(from) {
  ...
  // otherwise find it in the default schema dir
  if (await fs.stat(path.join(_defaultPrismaSchemaDir, 'schema.prisma')).catch(() => false)) {
    return _defaultPrismaSchemaDir
  }

  // otherwise find it in the default generated client dir
  if (await fs.stat(path.join(_defaultPrismaClientDir)).catch(() => false)) {
    return _defaultPrismaSchemaDir
  }
